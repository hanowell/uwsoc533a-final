...
* **Try to write your code as cleanly as possible, with comments about the steps you are taking along the way. The cleaner your code, the easier it is for me to see whether you got most of the steps right, and the more likely I will be able to give you partial credit for getting the code mostly right even if your numerical results are horribly wrong.**
...
**Calculate the TFR, GRR, NRR, GFR, and GFRc (GFR estimated from CBR assuming ${}_{\beta - n}C_\alpha = 0.25 = c$) for each of the years from 2005 through 2015. Yes, you can estimate each of these using the data provided, even if you have to do some of the estimation indirectly. Feel free to assume the sex ratio at birth (SRB) is a constant 1.05 males per female. Hint: It will be easier to get to NRR from GRR by applying SRB to fertility rates rather than trying to calculate it from TFR, SRB, and $p(A_m)$.**
...
* **`[tfr/grr/nrr/gfr/cbr/gfr_c]`: Five separate columns for each of the period rates requested, including the actual GFR and the GFT estimated assuming that ${}_{\beta - n}C_\alpha = 0.25 = c$.**
...
* **You will need to aggregate birth rates into five-year age groups. I suggest assuming that $\alpha = 15$ and $\beta = 55$, meaning you will need to calculate birth rates for five-year age groups from starting ages 15 your 50, in five-year increments.**
